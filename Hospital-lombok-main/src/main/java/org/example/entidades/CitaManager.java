...
public class CitaManager implements CitaService {
    ...
    private void validarCita(LocalDateTime fechaHora, BigDecimal costo) throws CitaException {
        if (fechaHora.isBefore(LocalDateTime.now())) throw new CitaException("No se puede programar una cita en el pasado.");
        if (costo.compareTo(BigDecimal.ZERO) <= 0) throw new CitaException("El costo debe ser mayor que cero.");
    }
    ...
    private boolean esMedicoDisponible(Medico medico, LocalDateTime fechaHora) {
        ...
        if (citasExistentes != null) {
            for (Cita citaExistente : citasExistentes) {
                long hours = Math.abs(Duration.between(citaExistente.getFechaHora(), fechaHora).toHours());
    ...
    public List<Cita> getCitasPorPaciente(Paciente paciente) {
        return citasPorPaciente.getOrDefault(paciente, Collections.emptyList());
    }

    @Override
    public List<Cita> getCitasPorMedico(Medico medico) {
        return citasPorMedico.getOrDefault(medico, Collections.emptyList());
    }

    @Override
    public List<Cita> getCitasPorSala(Sala sala) {
        return citasPorSala.getOrDefault(sala, Collections.emptyList());
    }

    @Override
    public void guardarCitas(String filename) throws IOException {
        try (PrintWriter writer = new PrintWriter(new FileWriter(filename))) {
            for (Cita cita : citas) writer.println(cita.toCsvString());
        }
    }
